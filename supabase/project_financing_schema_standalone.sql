-- Project Financing Data Table Schema (Standalone Version)
-- This version can be run independently without requiring loan_applications table
...
-- Create project_financing_data table
-- Note: loan_application_id is optional and can be linked later if loan_applications table exists
...
-- Optional: Add foreign key constraint to loan_applications if that table exists
-- Uncomment the following lines if you want to add the foreign key constraint later:
-- ALTER TABLE project_financing_data 
-- ADD CONSTRAINT fk_project_financing_loan_application 
